...
三个 Early Access 下载 CTA 只指向
`https://downloads.dougoos.com/early-access/macos/arm64/DougoOS.dmg`；登录、在线体验、
GitHub 与导航仍为无网络、无持久化的本地演示控件。Worker 实现 `GET/HEAD /v1/health`，
并让旧 `/install` 安装脚本稳定返回 `410`；其他 `/v1/*`（包括 ingest）返回 `404`。这里
没有账号、认证、数据库、队列、业务数据或遥测上报能力。
